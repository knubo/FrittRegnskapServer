...
class AccountBudget {
    ...
    function save($year, $budgetData) {
        ...
        $delPrep = $this->db->prepare("delete from " . AppConfig::pre() . "budsjett where year = ?");

        $delPrep->bind_params("i", $year);
        $delPrep->execute();

        $savePrep = $this->db->prepare("insert into " . AppConfig::pre() . "budsjett (year, post_type, amount, earning) values (?,?,?, ?)");
    ...
    function getAllBudgetYears() {
        $prep = $this->db->prepare("select distinct year from " . AppConfig::pre() . "budsjett order by year");
        return $prep->execute();
    }
    ...
    function getEarningsAndCostsFromGivenYear($year) {
        ...
        $prep = $this->db->prepare("select RP.post_type,sum(amount) as sumpost, year from " . AppConfig::pre() . "post RP, " . AppConfig::pre() . "line RL where RL.year = ? and RL.id=RP.line and RP.debet = ? and $addedWhere group by post_type order by post_type");
    ...
    function getEarningsAndCostsFromAllYears() {
        ...
        $addedWhere = "RP.post_type >= 4000 and RP.post_type <= 8500 and RP.post_type <> 8040";
        $prep = $this->db->prepare("select RP.post_type,sum(amount) as sumpost, year from " . AppConfig::pre() . "post RP, " . AppConfig::pre() . "line RL where RL.id=RP.line and RP.debet = ? and $addedWhere group by year, post_type order by post_type");
    ...
    function getMemberships($year) {
        $prep = $this->db->prepare("select * from " . AppConfig::pre() . "budget_membership where year = ?");
        $prep->bind_params("i", $year);
    ...
    function saveMemberships($memberships) {
        ...
        if (count($res) == 0) {
            $prep = $this->db->prepare("insert into " . AppConfig::pre() . "budget_membership set year=?");
            $prep->bind_params("i", $keyYear);
            $res = $prep->execute();
        }

        $prep = $this->db->prepare("update " . AppConfig::pre() . "budget_membership set fall_train=?,fall_course=?,fall_youth=?, spring_train=?,spring_course=?,spring_youth=?,year_members=?,year_youth=? where year=?");
