...
class AccountBudget {
    ...
    function getEarningsAndCostsFromGivenYear($year) {
        ...
        $prep = $this->db->prepare("select RP.post_type,sum(amount) as sumpost, year from " . AppConfig :: DB_PREFIX . "post RP, " . AppConfig :: DB_PREFIX . "line RL where RL.year = ? and RL.id=RP.line and RP.debet = ? and $addedWhere group by post_type order by post_type");
