...
class AccountHappening {
	...
	function save() {
		if ($this->Id) {
			$prep = $this->db->prepare("update " . AppConfig :: DB_PREFIX . "happening set description = ?, linedesc=?, debetpost=?, kredpost=?,count_req=?  where id = ?");
			$prep->bind_params("ssiiii", $this->description, $this->linedesc, $this->debetpost, $this->kredpost, $this->count_req, $this->Id);
			$prep->execute();
			return $this->db->affected_rows();
			
		} else {
			$prep = $this->db->prepare("insert into " . AppConfig :: DB_PREFIX . "happening (description, linedesc, debetpost, kredpost,count_req) values (?,?,?,?,?)");
