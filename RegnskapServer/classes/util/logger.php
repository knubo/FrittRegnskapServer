...
class Logger {
    ...
    function list_entries($pos) {
    	if(!$pos) {
    		$prep = $this->db->prepare("select * from " . AppConfig :: DB_PREFIX . "log order by id desc limit 30");
    	} else {
    		$prep = $this->db->prepare("select * from " . AppConfig :: DB_PREFIX . "log where id>= ? order by id desc limit 30");
